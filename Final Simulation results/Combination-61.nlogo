...
[
  ...
  newcomer-number
  repeated-collaborator-number
  non-repeated-collaborator-number
  check?                ;; check whether exist past-collaborator
  check2?               ;; check whether exist not-past-collaborator
]
...
to make-newcomer
  ...
  set newcomer-number newcomer-number + 1
end
...
to setup
  ...
  set newcomer-number 0
  set repeated-collaborator-number 0
  set non-repeated-collaborator-number 0
  set check? false
  set check2? false
...
to go
  if ticks = 10000
  [
    export-plot "% of agents in the giant component" "C:/Users/PC/Desktop/data/plot1.csv"
    export-plot "Link counts" "C:/Users/PC/Desktop/data/plot2.csv"
    stop
  ]
...
to pick-team-members
  ...
  ask new-team-member  ;; changing in-team? to true
  [
    set in-team? true
    set downtime 0
    set size 1.8
  ...
  ;; count new-team-member, whether incumbent or newcomer is greater
  repeat team-size - 1
  [
    (ifelse count(turtles with [in-team? and (incumbent?)]) > count(turtles with [in-team? and (not incumbent?)]) ;; apply incumbent p-rule (baseline)
    [                                                                                                             ;; toward own: 1, toward other: 0
      ;; apply incumbent-q-rule
      ;;                 non-repeated    repeated
      ;; toward own:          0             0
      ;; toward other:        1             0
      ;; step 1: Check whether in-team members have at least 2 incumbents, and all the in-team members are newcomers?

          ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? true]]
          if any? turtles with [check3?] [set check? true] ;; have link-neighbors, which represent in-team incumbent exists past-collaborator
          if any? turtles with [not check3? and not in-team?] [set check2? true] ;; not in-team and incumbent turtle neighbors, which mean not past-collaborator, therefore exist not past-collaborator
          ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? false]]
          ifelse (all? turtles with [in-team?] [color = 106]) or (check? = false or check2? = false)
          [
            set new-team-member one-of turtles with [not in-team?]
            set check? false
            set check2? false
          ...
          ;; apply q-rule
          ;; first, we randomly pick an in-team incumbent
          ;; we check whether the in-team incumbent has at least 1 repeated collaborator or not, if yes, we apply repeated collaborator q-rule; if no, we apply non-repeated collaborator q-rule.
          [
            set check? false
            set check2? false
            let selected-incumbent one-of turtles with [in-team? and incumbent?]
            ifelse any? (turtles with [in-team? and link-neighbor? selected-incumbent and incumbent?]) ;; exist repeated collaborator, apply repeated collaborator q-rule
            [  ifelse random-float 100.0 <= 50 ;; set new-team member repeated collaborator
               [
                 set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                 set repeated-collaborator-number repeated-collaborator-number + 1
               ]
               ;; set new-team member non-repeated collaborator
               [
                 set new-team-member one-of turtles with [not in-team? and not link-neighbor? selected-incumbent]
                 set non-repeated-collaborator-number non-repeated-collaborator-number + 1
               ]
            ]
            ;; not exist repeated collaborator, apply non-repeated collaborator q-rule
            [
                set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                set repeated-collaborator-number repeated-collaborator-number + 1
            ]
    ...
    (count(turtles with [in-team? and (not incumbent?)]) > count(turtles with [in-team? and (incumbent?)]));; apply newcomer p-rule
    [                                                                                                      ;; toward own: 0, toward other: 1
      ;; we recruit an incumbent as new-team-member
      ;; first, we have to check the two restrictions: (1) Are all the in-team member newcomer?
      ;;                                               (2) Do we have at least 2 in-team members are incumbent?

        ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? true]]
        if any? turtles with [check3?] [set check? true]  ;; exist past-collaborator
        if any? turtles with [not check3? and not in-team?] [set check2? true]    ;; exist not-pass-collaborator
        ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? false]]
        ifelse (all? turtles with [in-team?] [color = 106]) or (check? = false or check2? = false)
        [
            set new-team-member one-of turtles with [not in-team?]
        ...
        ;; we apply newcomer q-rule
        ;;                 non-repeated   repeated
        ;; toward own            0           0
        ;; toward other          1           0
        ;; first, we randomly pick an in-team incumbent
        ;; we check whether the in-team incumbent has at least 1 repeated collaborator or not, if yes, we apply repeated q-rule; if no, we apply non-repeated collaborator q-rule.
        [
            set check? false
            set check2? false
            let selected-incumbent one-of turtles with [in-team? and incumbent?]
            ...
            ;; no exist repeated collaborator, apply non-repeated collaborator q-rule.
            [
                set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                set repeated-collaborator-number repeated-collaborator-number + 1
            ]
    ...
    ;; tie:number of incumbent == number of newcomer, we randomly select incumbent p-rule or newcomer p-rule
    [
       ifelse random-float 100.0 >= 50 ;; select newcomer p-rule
       [
          ;; we recruit an incumbent as new-team-member
          ;; we first check the two restrictions

            ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? true]]
            if any? turtles with [check3?] [set check? true] ;; exist past-collaborator
            if any? turtles with [not check3? and not in-team?] [set check2? true] ;; exist not-past-collaborator
            ask turtles with [in-team? and incumbent?] [ask link-neighbors [set check3? false]]
            ifelse (all? turtles with [in-team?] [ color = 106 ]) or (check? = false or check2? = false)
            [
              set new-team-member one-of turtles with [not in-team?]
            ...
            ;; toward own       0            0
            ;; toward other     1            0
            ;; we first randomly pick an in-team incumbent
            ;; we check whether the in-team incumbent has at least a repeated-collaborator or not, if yes, we apply repeated collaborator q-rule, if no, we apply non-repeated collaborator q-rule
            [
                set check? false
                set check2? false
                let selected-incumbent one-of turtles with [in-team? and incumbent?]
                ifelse any? (turtles with [in-team? and link-neighbor? selected-incumbent and incumbent?]) ;; we apply repeated-collaborator q-rule
                [
                  ifelse random-float 100.0 >= 50 ;; we select a repeated-collaborator as new-team-member
                  [
                    set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                    set repeated-collaborator-number repeated-collaborator-number + 1
                  ]
                  ;; we select a non-repeated collaborator as new-team-member
                  [
                    set new-team-member one-of turtles with [not in-team? and not link-neighbor? selected-incumbent]
                    set non-repeated-collaborator-number non-repeated-collaborator-number + 1
                  ]
                ]
                ;; we apply non-repeated collaborator q-rule
                [
                    set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                    set repeated-collaborator-number repeated-collaborator-number + 1
                ]
       ...
       ;; select incumbent p-rule
       [
          ;; recruit an incumbent for new-team-member
          ;; we first check the two restrictions

              ask turtles with [in-team? and incumbent?] [ask link-neighbors [ set check3? true]]
              if any? turtles with [check3?] [set check? true] ;; exist past-collaborator
              if any? turtles with [not check3? and not in-team?] [set check2? true] ;; exist non-past-collaborator
              ask turtles with [in-team? and incumbent?] [ask link-neighbors [ set check3? false]]
              ifelse (all? turtles with [in-team?] [color = 106]) or (check? = false or check2? = false)
              [
                  set new-team-member one-of turtles with [not in-team?]
              ...
              ;; apply q-rule
              ;; we randomly pick an in-team incumbent, and check whether he/she has at least one repeated-collaborator, if yes, we apply repeated-collaborator q-rule, if no, we apply non-repeated collaborator q-rule
              ;;                non-repeated    repeated
              ;; toward own          0             0
              ;; toward other        1             0
              [
                  ...
                  ifelse any? (turtles with [in-team? and link-neighbor? selected-incumbent and incumbent?]) ;; we apply repeated collaborator q-rule
                  [
                    ifelse random-float 100.0 >= 50   ;; pick a repeated collaborator
                    [
                      set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                      set repeated-collaborator-number repeated-collaborator-number + 1
                    ]
                    ;; pick a non-repeated collaborator
                    [
                      set new-team-member one-of turtles with [not in-team? and not link-neighbor? selected-incumbent]
                      set non-repeated-collaborator-number non-repeated-collaborator-number + 1
                    ]
                  ]
                  ;; apply non-repeated collaborator q-rule
                  [
                      set new-team-member one-of turtles with [not in-team? and link-neighbor? selected-incumbent]
                      set repeated-collaborator-number repeated-collaborator-number + 1
                  ]
...
307
530
number of repeated collaborator
repeated-collaborator-number
17
1
11
...
507
530
number of non-repeated collaborator
non-repeated-collaborator-number
17
1
11
